...
async function sendToSheet(gasto) {
  try {
    const params = new URLSearchParams({
      ...
      fecha: gasto.fecha || "",
      descripcion: (gasto.descripcion || "").trim(), // ← NUEVO: descripción al Sheet
    });
...
async function sendToSheetNamed(sheet, gasto) {
  try {
    const params = new URLSearchParams({
      ...
      importe: String(parseFloat(gasto.importe) || 0),
      fecha: gasto.fecha || "",
      descripcion: (gasto.descripcion || "").trim(), // ← NUEVO
    });
    await fetch(SCRIPT_URL + "?" + params.toString(), {method:"GET", mode:"no-cors"});
    return true;
  } catch { return false; }
...
export default function App() {
      ...
      try {
        ...
        const parsed = rows
          ...
          .map((r, i) => ({
            ...
            descripcion: String(r[5] || r[1]).trim(), // ← columna F del Sheet si existe
          }))
  ...
  function exportCSV() {
    const rows = userGastos.map(g=>[g.id,g.fecha,g.categoria,g.subcategoria,g.persona,g.importe,g.descripcion||""].join(","));
    const csv = "ID,FECHA,CATEGORIA,SUBCATEGORIA,PERSONA,IMPORTE,DESCRIPCION\n" + rows.join("\n");
  ...
  // ── NUEVO: datos para gráfica de subcategorías ──
  const bySubcat = (() => {
    const map = {};
    filteredGastos.forEach(g => {
      const key = g.subcategoria || g.categoria;
      if (!map[key]) map[key] = { name: key, total: 0, color: CAT_COLORS[g.categoria] || "#888" };
      map[key].total += g.importe;
    });
    return Object.values(map).sort((a,b)=>b.total-a.total);
  })();
  const topSubcat = bySubcat.slice(0, 12);
      ...
      <div style={{maxWidth:640,margin:"0 auto",padding:"18px 14px 40px"}}>
                ...
                <div key={e.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:`1px solid ${dm?"#1E1E2E":"#F0F0F8"}`}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,fontWeight:600,color:dm?"#FF6B6B":"#1A1A2E",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                      ...
                      {/* ── NUEVO: mostrar descripción si existe y es distinta a subcategoría ── */}
                      {e.descripcion && e.descripcion !== e.subcategoria && (
                        <span style={{fontWeight:400,color:dm?"#9090BB":"#7070A0",fontSize:12}}> · {e.descripcion}</span>
                      )}
                    </div>
        ...
        {tab==="dashboard" && (
          <div>
            ...
            <Sec dm={dm} title="Adri vs Mari por mes">
              <ResponsiveContainer width="100%" height={165}>
                <BarChart data={byMonthData} margin={{top:4,right:0,left:-22,bottom:0}}>
                  ...
                  <Tooltip formatter={v=>fmt(v)} contentStyle={{background:"#FFFFFF",border:"1px solid #8080CC",borderRadius:8,color:"#1A1A2E",fontFamily:"monospace",fontSize:13,fontWeight:600}}/>
            ...
            {/* ── NUEVO: Pie subcategorías ── */}
            {topSubcat.length>0 && (
              <Sec dm={dm} title="Por subcategoría">
                <ResponsiveContainer width="100%" height={170}>
                  <PieChart>
                    <Pie data={topSubcat} dataKey="total" nameKey="name" cx="50%" cy="50%" outerRadius={72} innerRadius={36}>
                      {topSubcat.map((c,i)=><Cell key={i} fill={c.color}/>)}
                    </Pie>
                    <Tooltip formatter={v=>fmt(v)} contentStyle={{background:"#FFFFFF",border:"1px solid #8080CC",borderRadius:8,color:"#1A1A2E",fontFamily:"monospace",fontSize:13,fontWeight:600}}/>
                  </PieChart>
                </ResponsiveContainer>
                <div style={{display:"flex",flexDirection:"column",gap:5,marginTop:4}}>
                  {topSubcat.slice(0,10).map(c=>(
                    <div key={c.name} style={{display:"flex",alignItems:"center",gap:8}}>
                      <div style={{width:8,height:8,borderRadius:2,background:c.color,flexShrink:0}}/>
                      <span style={{flex:1,fontSize:12,color:"#4A4A6A"}}>{c.name}</span>
                      <span style={{fontSize:12,fontFamily:"monospace",color:"#1A1A2E"}}>{fmt(c.total)}</span>
                ...
                <div key={g.id} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 0",borderBottom:"1px solid #0C0C16"}}>
                  ...
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,color:"#2A2A3E",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                      ...
                      {/* ── NUEVO: descripción en historial ── */}
                      {g.descripcion && g.descripcion !== g.subcategoria && (
                        <span style={{color:"#7070A0",fontSize:11}}> · {g.descripcion}</span>
                      )}
                    </div>
